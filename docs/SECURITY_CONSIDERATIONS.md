...
- **Comments are unauthenticated free text** — `Comment.body` is stored and rendered as-is (React escapes it on render, so this isn't an XSS hole today, but there's no server-side sanitization/length limit either), and since there's no auth, `DELETE /api/comments/:id` doesn't check that the requester authored the comment — anyone can delete anyone's comment. Same "no auth (MVP)" tradeoff as the rest of the app, not a separate gap.
