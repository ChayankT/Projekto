...
- `UserStory` carries a free-form `tags: [String]` field, editable as a comma-separated list on the story form and shown as badges on cards; it's included in the board's search matching. Tags are also a first-class filter: a chip row above the board lists every distinct tag in use across the project's stories, and clicking a chip (or a tag badge directly on a card) filters the board to that tag — clicking the active chip again clears it. The chip list is built from all of a project's stories, not just the currently-visible ones, so it doesn't shrink as soon as a filter narrows the board down. Because the chip list spans the whole project but the board defaults to "Backlog only", activating a tag filter also widens the sprint-scope dropdown to "Any sprint" — otherwise a tag used only by sprinted stories would filter the backlog-only board down to nothing.
